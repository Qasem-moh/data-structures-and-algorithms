...
const replaceZeros = (string) => {
  const replaceValue = /0/g;
  return string.replace (replaceValue, 'zero');
}
...
const validatePin = (pin) => {
  const numericalPattern = /^(\d{4}|^\d)$/;
  return numericalPattern.test(pin);
};
...
const validateWord = (word) => {
  const expressionPattern =/\b[a-zA-Z]{5,10}\b/;
  return expressionPattern.test(word);
};
...
const validatePhoneNumber = (phoneNumber) => {
  const phoneNoPattern = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
  return phoneNoPattern.test(phoneNumber);
};
