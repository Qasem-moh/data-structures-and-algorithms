...
const isNum = (input) => {
  const contanier = /[0-9]/g
  return contanier.test(input)
};
...
const containsWorld = (input) => {
  const chicking = /(world)/g
  return chicking.test(input)
};
...
const isCapitalized = (str) => {
  const checking = /[A-Z][a-z]+/g;
  if (str.match(checking))
    return str.match(checking)
  else
    return []
};
